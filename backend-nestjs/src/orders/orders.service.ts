...
import { LeagueStatus, OrderType } from '@prisma/client';
...
export class OrdersService {
  ...
  async create(userId: string, createOrderDto: CreateOrderDto) {
    ...
    const quote: { price: number } = await this.marketService.getQuote(symbol);
    ...
    if (portfolio.userId !== userId) {
      throw new ForbiddenException(
        'You can only store orders for your own portfolio',
      );
    }

    if (
      portfolio.league.status === LeagueStatus.COMPLETED ||
      portfolio.league.status === LeagueStatus.CANCELLED
    ) {
      throw new BadRequestException('Trading is closed for this league');
    }
      ...
      if (type === OrderType.BUY) {
        ...
        if (currentPortfolio.cashBalance < totalCost) {
          throw new BadRequestException(
            `Insufficient cash balance for estimated $${totalCost.toFixed(2)}`,
          );
        }
