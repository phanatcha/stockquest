...
export class MarketService {
    async getQuote(symbol: string) {
        try {
            ...
            if (!result || !result.regularMarketPrice) throw new Error("No price Data");
            return {
            ...
            throw new BadRequestException(`Fetch failed for ${symbol}: ${e.message || String(e)}`);
        }
    }

    async getHistory(symbol: string, interval: '1m' | '5m' | '1d' | '1wk' | '1mo' = '1d') {
        try {
